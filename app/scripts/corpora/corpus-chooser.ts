...
export const updateLimitedAccess = (node: ChooserFolder, credentials: string[] = [], protectedCorpora: string[] = []): boolean => {
    console.log(`=== updateLimitedAccess called, credentials=${credentials}, protectedCorpora=${protectedCorpora} ===`)
    ...
    for (const corpus of node.corpora) {
        ...
        if (corpusLimitedAccess) {
            console.log(`Corpus ${corpus.id}: limited_access=${corpusLimitedAccess}, userHasAccess=${corpus.userHasAccess}`)
        }
